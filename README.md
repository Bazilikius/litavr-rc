...
На багатьох платах **ESP32 Dev Module** вивід **GPIO 2** є критичним strapping-піном, до якого також підключено вбудований синій світлодіод. Підключення LoRa `DIO0` до GPIO 2 блокує перехід плати у режим прошивки (Upload).

* **Вирішення:** У цій системі вивід **LORA_DIO0 повністю перенесено на безпечні, не-strapping піни**:
  - На приймачі (RX) — на **GPIO 16**.
  - На передавачі (TX) — на **GPIO 21**.
  Це на 100% розв'язує проблему блокування прошивки та забезпечує миттєвий, стабільний запуск і легке завантаження через USB-UART на будь-якому ПК без будь-яких фізичних відключень дротів!
