...
Це сучасна, високонадійна бездротова система керування з резервуванням та автоматичним контролем безпеки на базі плат **ESP32 Dev Module** та модулів **SX127x LoRa** (програмується у середовищі **Arduino IDE / C++**).
...
## ⚠️ Усунення проблем із запуском плати (Standalone Power / Живлення без ПК)

Якщо ваша плата ESP32 стабільно запускається при підключенні через USB до комп'ютера, але **зависає і не стартує при автономному живленні** (від зовнішнього блока живлення 5В, акумулятора чи BEC), це типова проблема ланцюга автоскидання (Auto-Reset) більшості плат ESP32 Dev Module.

Ось 3 перевірені кроки для вирішення цієї проблеми:

### 1. Налаштування в Arduino IDE (Програмне рішення):
* У меню інструментів Arduino IDE знайдіть пункт **"USB CDC On Boot"** (або "USB CDC на завантаженні").
* Переконайтеся, що цей пункт встановлено у стан **"Disabled"** (Вимкнено).
* *Пояснення:* Якщо цей режим увімкнено, ESP32 під час старту очікує ініціалізації USB-порту від ПК і блокує виконання програми, якщо кабель підключений лише до джерела живлення без активного обміну даними.

### 2. Встановлення конденсатора на пін EN (Апаратне рішення):
* Підключіть електролітичний або керамічний **конденсатор ємністю від 0.1 мкФ до 10 мкФ** (рекомендовано **10 мкФ**) між виводом **EN (CHIP_PU / RST)** та **GND** на вашій платі ESP32.
* *Пояснення:* При подачі живлення від батареї напруга наростає повільніше, ніж при підключенні до ПК. Конденсатор затримує наростання напруги на виводі EN, створюючи чіткий імпульс скидання (Reset), що дозволяє процесору успішно та стабільно стартувати автономно.

### 3. Запобігання конфліктам завантаження (Strapping Pins):
* Завдяки перенесенню виводу **LORA_DIO0 на GPIO 34** (який є винятково вхідним портом `input-only`):
  - Повністю усунено вплив виходів LoRa на strapping-піни (GPIO 2, GPIO 12).
  - ESP32 більше не намагається увійти в режим прошивки при подачі живлення 3.3В на LoRa-модуль.
