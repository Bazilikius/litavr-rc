...
## ⚡️ Важливе апаратне зауваження: Запобігання конфліктам прошивання та завантаження (GPIO 2)

На багатьох платах **ESP32 Dev Module**:
* **GPIO 2** та **GPIO 12** є **strapping pins**, стан яких зчитується процесором при старті для переходу в режим прошивки. Крім того, до GPIO 2 на більшості плат підключений **вбудований синій світлодіод**.
...
* **Вирішення (Якщо виникає помилка прошивки):**
  1. Тимчасово відключіть провід `DIO0` LoRa від плати ESP32 перед завантаженням прошивки.
  2. У меню інструментів Arduino IDE переконайтеся, що параметр **"USB CDC On Boot"** встановлено у стан **"Disabled"** (Вимкнено). Це гарантує, що плата буде успішно запускатися від автономного живлення (BEC/акумулятора) без підключеного комп'ютера.
  3. Для стабілізації ланцюга автоскидання при автономному живленні рекомендується підключити конденсатор ємністю **10 мкФ** між контактами **EN (RST)** та **GND** на платі ESP32.
...
* **CS / SS** -> GPIO 5
* **RST**   -> GPIO 14
* **DIO0 (Приймач RX)** -> **GPIO 2** *(Підключення DIO0 до GPIO 2. Якщо плата не прошивається — тимчасово відключіть провід DIO0 під час прошивання!)*
* **DIO0 (Передавач TX)** -> **GPIO 2** *(Підключення DIO0 до GPIO 2. Якщо плата не прошивається — тимчасово відключіть провід DIO0 під час прошивання!)*
...
- Калібрування Min/Max кутів сервоприводів та их швидкості руху.
